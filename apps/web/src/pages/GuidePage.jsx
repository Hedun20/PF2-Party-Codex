  ...
  return (
    <div className="page-stack">
      <header className="list-header guide-hero article-page-header">
        ...
        <Link className="gold-button magic-liquid-button" to="/editor"><PenLine size={16} /> <span>Создать первую статью</span></Link>
      </header>
      ...
      <section className="guide-grid guide-steps-grid">
        {steps.map(([num, title, text, href, Icon]) => (
          <article key={title}>
            ...
            <h2>{title}</h2>
            <p>{text}</p>
            <Link className="upload-button" to={href}>Открыть</Link>
          </article>
        ))}
      </section>
      ...
      <section className="builder-section guide-deep-dive">
        ...
        <article>
          <Eye size={22} />
          <h2>Редактор</h2>
          <p>В статье кнопка “Редактор” находится справа. Внутри есть два рабочих режима: визуальный редактор для мастера и raw Markdown для точной ручной правки.</p>
        </article>
      </section>
    </div>
  );

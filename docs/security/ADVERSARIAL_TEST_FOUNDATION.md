...
- prompt/evidence separation and bounded analytics metadata checks;
- recursive evidence redaction and a controlled `SecurityAssertionError`.
...
The negative fixtures include cross-tenant IDs, inactive membership, another player's knowledge, GM-only markers, secret roll data, spoofed Discord binding, replayed Foundry evidence, a tampered signed batch, duplicate job effects, incomplete cache identity, an external deep link, prompt injection, sensitive analytics metadata, cross-campaign export/delete, and a hidden achievement title.

If a future edit weakens an assertion so its insecure fixture passes, `tests/security/security-foundation.test.mjs` fails. A separate test inspects every controlled error message and fails if a credential/private marker, Mongo password, Bearer value, email, or other forbidden evidence is present.
...
6. add the scenario to `REQUIRED_SECURITY_SCENARIOS` if it creates a new boundary;

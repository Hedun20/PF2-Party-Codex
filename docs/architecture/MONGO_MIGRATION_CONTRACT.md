...
`parseMigrationCommandRequest`, `parseVerifiedMigrationCommandRequest`, `parseMigrationManifest`, `parseMigrationRestoreDrill` and `parseMigrationReport` make the operational boundary executable. The ordinary request parser accepts only `inventory`/`dryRun`; commit, verify and rollback must use the verified parser with the actual manifest, restore-drill record, trusted evaluation time and a server-supplied SHA-256 implementation. Report parsing likewise requires the trusted approved manifest, trusted evaluation time and server-supplied SHA-256 implementation. Commit/verify/rollback reports must also receive the actual restore-drill evidence; inventory/dry-run reports must receive `null`. A report cannot self-attest which plan or backup gate it completed.
...
The executable `hed19-restore-drill-v1` record binds `restoreRef`, migration/version, source snapshot/fingerprint and canonical manifest hash. It also requires the Mongo server and database-tool versions, encrypted-backup/key version, artifact checksum, typed consistency-boundary kind/reference, isolated restored-database fingerprint, operator/automation ID, measured restoration duration, temporary-target expiry, and literal proof that production routing is disabled. The parser requires passing `BACKUP_ARTIFACT_CHECKSUM_VERIFIED`, `BACKUP_ENCRYPTION_VERIFIED`, `SNAPSHOT_CONSISTENCY_VERIFIED`, `RESTORE_TARGET_ISOLATED`, `PRODUCTION_ROUTING_BLOCKED`, `RESTORED_COUNTS_MATCH` and `RESTORED_INDEXES_MATCH` invariants; nonzero collection/index counts; and `status=succeeded` with `fatalIssueCount=0`. `parseVerifiedMigrationCommandRequest` compares every binding and rejects future-completed or expired drills at the trusted server evaluation instant.

...
| 3. Session compatibility | Implement the approved principal/cookie gateway and typed campaign-context adapter while preserving legacy Bearer clients. After the pre-foundation gate passes, this phase may scaffold only the thin `apps/web-next` auth/session shell required to exercise the post-foundation browser and rollback gate; it may not add protected campaign routes or receive traffic. | The pre-foundation server/campaign table passes before the thin shell begins. Login/logout/reset/revoke, cookie flags/expiry/rotation, CSRF, header/route disagreement, and cross-campaign denial tests pass; then the actual thin shell passes the post-foundation browser/rollback table before Phase 3 exits. | Feature flag or route rollback to Bearer-only Express; invalidate new cookies. The un-routed thin shell can be discarded without moving traffic. |
| 4. Next foundation | After Phase 3 exits, expand the already-proven thin auth/session shell with root layouts, error/loading boundaries, observability, config, and CI strict checks. No broad screen port. | Production build, smoke route, CSP/security headers, typed env, and deploy preview pass. No Mongo access from presentation code. | Do not route production traffic to the app. |

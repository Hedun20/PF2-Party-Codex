...
Before the Vite artifact is frozen for rollback, it adds a synchronous pre-module bootstrap and a non-secret `pf2-auth-token-version` marker. The expected active legacy-key version is baked into that artifact. Before the API client module can load or make any request, the bootstrap removes `pf2-auth-token` and its marker when the marker is missing or differs, then presents login. A successful Vite password login stores the new token and matching version together. Storage access failure stops startup on a static recovery screen; it never continues with an unclassified token. Retired key-version identifiers are never reused.
...
2. **Dual issuance on Vite:** use `vite-dual`; password login returns the legacy token to Vite and also sets the cookie. Add an authenticated Bearer-to-cookie exchange for already logged-in Vite sessions. The exchange returns no token body. Add and verify the pre-module token-version bootstrap, then freeze this rollback-compatible Vite artifact. Same-user dual mutation compatibility applies only in this profile.
3. **Pre-cutover rollback rehearsal:** in disposable automation, leave a retired token in browser storage, switch atomically from `next-cookie-preferred` to `vite-dual` with a newly rotated legacy signing key, and prove the bootstrap removes the stale token before the first network request. A cookie-only user with no accepted Bearer must then see Vite login, reauthenticate with the password flow and its existing rate/enumeration controls, receive/store a new-version Bearer, and complete an authenticated Vite mutation. No cookie-to-Bearer endpoint exists.
...
| Rollback stale-token lockout | A retired/missing-version `pf2-auth-token` is removed before any Vite module/network request; password login succeeds without using attached credentials, while protected invalid dual input still fails closed |
...
2. Add the Express cookie/CSRF/legacy gateway behind `vite-bearer`, including Vite-only dual-mutation compatibility, profile/key-version enforcement, the pre-request stale-token bootstrap, credential-independent password login, compatibility-wide logout and the reauthentication-based rollback rehearsal. Do not add a cookie-to-Bearer route. Exercise the session-specific rows, but do not claim the combined Phase 3 gate yet.
